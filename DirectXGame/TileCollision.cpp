...
void ResolveEnemyVsTilemap(Enemy& enemy, const TileField& tf) {
	...
	WorldTransform& wt = enemy.EditWorldTransform();
	Vector3& vel = enemy.EditVelocity();
	Vector3 pos = wt.translation_;

	const Vector3 eh = EnemyHalfExtents();
	const float BHX = tf.pitchX * 0.5f;
	const float BHY = tf.pitchY * 0.5f;
	const float skin = SkinWidth();
	...
	auto collideAxis = [&](int axis) {
		...
		for (int ty = cy - 3; ty <= cy + 3; ++ty) {
			for (int tx = cx - 3; tx <= cx + 3; ++tx) {
				...
				float cxw = tf.originX + tx * tf.pitchX;
				float cyw = tf.originY + ty * tf.pitchY;

				const float dx = pos.x - cxw;
				const float dy = pos.y - cyw;
				const float px = (BHX + eh.x) - std::fabs(dx);
				const float py = (BHY + eh.y) - std::fabs(dy);
				if (px <= 0.0f || py <= 0.0f)
					continue;
